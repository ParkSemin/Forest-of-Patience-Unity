...
public class GameManagerSingle : MonoBehaviour
{
    ...
    void Start()
    {
        bestMin = PlayerPrefs.GetInt(SceneManager.GetActiveScene().name + "_BestMin");
        bestSec = PlayerPrefs.GetInt(SceneManager.GetActiveScene().name + "_BestSec");
    ...
    void Update()
    {
        if (isGameover)
        {
            ...
            if (!GameManagerSingle.instance.isPause)
                Timer();
    ...
    public void OnPlayerFinish() {
        ...
        if (bestMin*60 + bestSec > min*60 + (int)sec) {
            bestMin = min;
            bestSec = (int)sec;
            PlayerPrefs.SetInt(SceneManager.GetActiveScene().name + "_BestMin", bestMin);
            PlayerPrefs.SetInt(SceneManager.GetActiveScene().name + "_BestSec", bestSec);
        }
